...
class LLMJudge:
    """LLM-as-Judge 評価器（Bedrock Claude Haiku）"""
    ...
    def _call_bedrock_sync(self, prompt: str) -> dict:
        """Bedrock Converse APIで構造化出力を取得する（同期）。"""
        resp = self._client.converse(
            modelId=self._model,
            system=[
                {
                    "text": "あなたは車両問診AIの品質評価の専門家です。客観的かつ厳格に評価してください。"
                    "必ずjudge_evaluationツールを使って結果を返してください。"
                }
            ],
            messages=[{"role": "user", "content": [{"text": prompt}]}],
            toolConfig={
                "tools": [_BEDROCK_TOOL],
                "toolChoice": {"tool": {"name": "judge_evaluation"}},
            },
    ...
    async def evaluate(
        ...
        try:
            ...
            result = await loop.run_in_executor(
                None, partial(self._call_bedrock_sync, prompt)
            )

            if not result:
                raise ValueError("Bedrock returned no tool_use result")
            ...
            # Ensure step_comparison exists
            if "step_comparison" not in result:
