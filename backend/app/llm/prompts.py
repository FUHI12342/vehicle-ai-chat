...
- choices は質問の回答として直接適切なもののみ。質問と無関係な選択肢は絶対に入れないこと
  例: 質問「エンジンはかかっていますか？」→ choices: ["かかっている", "かかっていない", "たまにかかる"]
  NG: 質問「エンジンはかかっていますか？」→ choices に「エンジン内部異常」など質問の答えでないものを入れる
- choices に重複を入れないこと
- choices のラベルは素人が見てすぐ意味がわかるように。専門用語にはカッコ書きで説明を添えること
  NG: "走行中" → OK: "走行中（動いているとき）"
  NG: "低頻度" → OK: "たまに（週1回以下）"
  マニュアルの用語を使う場合もカッコ書きで素人向けの説明を添えること
  各ラベルは20文字以内を目安に
