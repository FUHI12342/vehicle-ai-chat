...
def _safe_float(val: Any, default: float = 0.0) -> float:
    """LLMが返す confidence 値を安全に float に変換する。"""
    try:
            ...
            for line in source.content.split("\n"):
                ...
                if re.match(r"^\d+[.、）]", stripped) or stripped.startswith("- "):
                    ...
                    if not text or text in steps:
                        continue
                    # 不完全な行をフィルタ: 文末が句点・動詞終止形等で終わらず、
                    # 助詞・接続助詞・連用形で終わっている場合はスキップ
                    if len(text) < 5:
                        continue
                    # 完全な文の判定: 動詞終止形、命令形、「ください」等で終わるか
                    _complete_endings = ("る", "す", "く", "む", "ぶ", "つ", "ぬ", "う",
                                         "ます", "ください", "こと", "した", "する",
                                         "です", "せん", "ない")
                    _incomplete_endings = ("し、", "き、", "に、", "を、", "の、",
                                           "を", "に", "の", "が", "で", "と", "は",
                                           "から", "まで", "より")
                    # 助詞で終わる → 明らかに不完全
                    if text.endswith(_incomplete_endings) and not text.endswith("ください"):
                        continue
                    # 完全な動詞形で終わらず、カタカナ・漢字の名詞で終わる → 不完全の可能性
                    # ただし「〜を外す」「〜に入れる」等の動詞は許可
                    last_char = text[-1]
                    if (last_char not in ("。", "）", ")", "」")
                            and not text.endswith(_complete_endings)
                            and not re.search(r'[ぁ-ん]$', text)):
                        # カタカナや漢字で終わる短い行 → 途中切れの可能性高い
                        continue
                    steps.append(text)
    return steps
...
def _build_additional_instructions(
    ...
    # 直前の回答を最優先位置に明示（Haiku が見落とさないよう冒頭に配置）
    if session.diagnostic_path:
        last_entry = session.diagnostic_path[-1]
        last_q = last_entry.get("q", "")
        last_a = last_entry.get("a", "")
        if last_q and last_a:
            parts.append(
                f"\n\n【確定情報・再質問厳禁】\n"
                f"直前の質問「{last_q}」に対し、ユーザーは「{last_a}」と回答済みです。\n"
                f"この回答は確定事実です。同じ内容を再度質問しないでください。\n"
                f"この回答を前提として次の診断ステップに進んでください。"
            )
    ...
    if session.guide_phase == "guiding":
        ...
        # キャッシュ優先、なければRAGから手順リストを抽出
        procedure_steps = session.guide_cached_steps or _extract_procedure_steps(rag_sources or [])
...
def _repair_json(raw: str) -> dict | None:
    """Haiku出力の不正JSONを修復する。成功時はdictを返す。"""
    # 1. strict=False で制御文字を許容
    try:
        return json.loads(raw, strict=False)
    except json.JSONDecodeError:
        pass

    # 2. 最後の } までを切り出す（後続テキストを除去）
    last_brace = raw.rfind("}")
    if last_brace > 0:
        try:
            return json.loads(raw[: last_brace + 1], strict=False)
        except json.JSONDecodeError:
            pass

    # 3. 欠損カンマを補完: }{ や }" の間にカンマを挿入
    fixed = re.sub(r'"\s*\n\s*"', '",\n"', raw)
    fixed = re.sub(r'}\s*\n\s*"', '},\n"', fixed)
    try:
...
async def _llm_call(provider, diagnostic_prompt: str, max_retries: int = 2) -> dict:
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            # JSON修復を試行
            repaired = _repair_json(raw) if raw else None
            if repaired:
                repaired = _unwrap_schema_response(repaired)
                if repaired.get("message"):
                    logger.info("JSON repair succeeded (attempt %d/%d)", attempt + 1, max_retries)
                    return repaired
            logger.warning(
                "LLM returned invalid JSON (attempt %d/%d): %s. Raw: %s",
                attempt + 1, max_retries, e, raw[:200] if raw else "N/A",
            )
    ...
    # Ultimate fallback — 具体的なエラーメッセージ（「他に気になる症状」は出さない）
    return {
        "action": "ask_question",
        "message": "申し訳ありません。回答の生成中にエラーが発生しました。もう一度お試しください。",
        "choices": ["もう一度試す"],
...
async def handle_diagnosing(session: SessionState, request: ChatRequest) -> ChatResponse:
    ...
    # ---------------------------------------------------------------
    # 自由テキストの解決報告 → DONE（ガイド完了後のみ）
    _RESOLUTION_EXACT = ("解決しました", "解決した", "直りました", "治りました", "動きました")
    if user_input in _RESOLUTION_EXACT and session.guide_phase in ("guiding", "guide_offered"):
        logger.info("Free-text resolution detected: %s (phase=%s)", user_input, session.guide_phase)
        session.current_step = ChatStep.DONE
        return ChatResponse(
            session_id=session.session_id,
            current_step=ChatStep.DONE.value,
    ...
    try:
        ...
        # ガイドモード中は手順ステップをフォールバック表示
        if session.guide_phase == "guiding":
            procedure_steps = _extract_procedure_steps(rag_sources)
            guide_turns = session.diagnostic_turn - session.guide_start_turn
            if procedure_steps and guide_turns < len(procedure_steps):
                fallback_msg = _format_guide_step_message(procedure_steps[guide_turns])
                fallback_choices = _append_default_choices(["できました", "うまくいかない"])
                session.conversation_history.append({"role": "assistant", "content": fallback_msg})
                return ChatResponse(
                    session_id=session.session_id,
                    current_step=ChatStep.DIAGNOSING.value,
                    prompt=PromptInfo(type="single_choice", message=fallback_msg, choices=fallback_choices),
                    manual_coverage=session.manual_coverage,
                    diagnostic_turn=session.diagnostic_turn,
                )
        fallback_msg = "申し訳ありません。回答の生成に失敗しました。もう一度お試しください。"
    ...
    if (session.guide_phase == "guiding"
        ...
        # ガイド初回ターンでRAGから手順をキャッシュ（以降のターンではキャッシュを使用）
        if not session.guide_cached_steps:
            cached = _extract_procedure_steps(rag_sources)
            if cached:
                session.guide_cached_steps = cached
                logger.info("Guide mode: cached %d procedure steps", len(cached))

        procedure_steps = session.guide_cached_steps
        estimated_steps = len(procedure_steps) if procedure_steps else _count_procedure_steps(rag_sources)
        ...
        if (action == "provide_answer"
                ...
                and session.diagnostic_turn < session.max_diagnostic_turns):
            logger.warning(
            ...
            if procedure_steps and step_idx < len(procedure_steps):
                ...
                logger.info(
                    "Guide guard: rewrote message to step %d/%d: %s",
                    step_idx + 1, len(procedure_steps), target_step,
                )
            elif procedure_steps and step_idx >= len(procedure_steps):
                # キャッシュのステップを全て完了 → 解決確認へ
                action = "provide_answer"
                logger.info("Guide guard: no more steps, allowing provide_answer")
            else:
                # キャッシュにステップがない場合、LLMの元メッセージを活用
                # 「完了しましたか？」形式に変換
                if message and not message.endswith("完了しましたか？"):
                    message = message.rstrip("。、\n ") + "\n\n完了しましたか？"
                choices = ["できました", "うまくいかない"]
                logger.info("Guide guard: using LLM message as step (no cached steps)")

    # 8. Save rewritten_query, confidence, and manual_coverage
    session.rewritten_query = result.get("rewritten_query", "")
    session.last_confidence = _safe_float(result.get("confidence_to_answer"), 0.0)
    ...
    if action == "ask_question" and question_topic:
        ...
        if _is_irrelevant_topic(question_topic, symptom_text, session.conversation_history):
            ...
            try:
                ...
                session.last_confidence = _safe_float(result.get("confidence_to_answer"), session.last_confidence)
                question_topic = result.get("question_topic", "")
            except Exception as e:
    ...
    if action == "provide_answer":
        ...
        if session.guide_phase == "guide_offered":
            ...
            # 1) 対処不可を示す否定語があればガイド不要
            _no_fix_indicators = (
                "見つからなかった", "見つかりません", "対処方法はありません",
                "該当する記載がない", "記載が見つかりません",
                "ディーラーでの点検をお勧め", "販売店での点検をお勧め",
                "ディーラーへの相談を推奨", "販売店で点検を受ける",
            )
            has_no_fix = any(neg in message for neg in _no_fix_indicators)
            # 2) 手順言及キーワード（否定語がなければガイド可能と判断）
            has_procedure_kw = any(
                kw in message
                for kw in ("手順が記載", "手順に従", "手順として", "操作を行", "解除の手順", "解除穴")
            )
            has_procedure = has_procedure_kw and not has_no_fix
            if has_procedure:
                transition_choices = [
    ...
    # ---------------------------------------------------------------
    # 12. Unified duplicate guard (ask_question + clarify_term)
    # ---------------------------------------------------------------
    if (action in ("ask_question", "clarify_term")
            and _is_duplicate_question(message, session.last_questions)
            and session.guide_phase != "guiding"):
        logger.warning("Duplicate question detected: %s", message[:80])
        # Re-call LLM with explicit anti-repeat instruction
        last_path = session.diagnostic_path[-1] if session.diagnostic_path else {}
        anti_repeat_prompt = (
            diagnostic_prompt
            + f"\n\n【重要・前回と同じ質問は禁止】\n"
            f"ユーザーは「{last_path.get('a', user_input)}」と既に回答しました。\n"
            f"この回答を踏まえて、別の角度から確認するか、"
            f"確信度が十分であれば provide_answer で診断結果を報告してください。\n"
            f"同じ質問や類似の質問を絶対に繰り返さないでください。"
        )
        try:
            retry_result = await _llm_call(provider, anti_repeat_prompt)
            retry_msg = retry_result.get("message", "")
            if not _is_duplicate_question(retry_msg, session.last_questions):
                action = retry_result.get("action", action)
                message = retry_msg
                choices = retry_result.get("choices")
                urgency_flag = retry_result.get("urgency_flag", urgency_flag)
                can_drive_llm = retry_result.get("can_drive", can_drive_llm)
                session.rewritten_query = retry_result.get("rewritten_query", "")
                session.last_confidence = _safe_float(
                    retry_result.get("confidence_to_answer"), session.last_confidence
                )
                question_topic = retry_result.get("question_topic", "")
                logger.info("Duplicate retry succeeded: action=%s", action)
            else:
                # Retry also duplicated — use a non-duplicate fallback question
                last_a = last_path.get("a", user_input)
                message = (
                    f"「{last_a}」の状況ですね。承知しました。\n\n"
                    "この症状はいつ頃から発生していますか？"
                )
                choices = ["最近（1週間以内）", "1ヶ月ほど前から", "以前からずっと"]
                action = "ask_question"
                logger.warning("Duplicate retry also failed, using fallback question")
        except Exception as e:
            logger.warning("Duplicate retry LLM call failed: %s", e)
            last_a = (session.diagnostic_path[-1].get("a", user_input)
                       if session.diagnostic_path else user_input)
            message = (
                f"「{last_a}」の状況ですね。\n\n"
                "この症状はいつ頃から発生していますか？"
            )
    ...
    # Handle action changes from duplicate retry (provide_answer/escalate can't
    # reach the main handlers above, so dispatch them here)
    if action == "provide_answer":
        session.conversation_history.append({"role": "assistant", "content": message})
        if session.guide_phase == "identifying":
            session.identified_issue = message
            session.guide_phase = "guide_offered"
            session.rag_answer = message
            transition_choices = [
